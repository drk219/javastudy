...
public class MainClass {
  ...
  public static void method1() {  // 디렉터리 생성/삭제 (꼭 알아야함!)
    ...
    // 디렉터리 유무에 따른 생성
    if(!dir.exists()) {     // 디렉터리가 없으면 ('!' 또는 '==false' 사용)
      dir.mkdirs();
    } else {                // 디레터리가 있으면
  ...
  public static void method2() {  // 파일 생성/삭제
  ...
  public static void method3() {  // 파일 클래스 주요 메소드
    
    /*
     * File 클래스 주요 메소드
     * 1. listFiles()    : 디렉터리 내의 모든 File 객체를 File[] 배열로 반환
     * 2. getName()      : 이름 반환
     * 3. getParent()    : 저장된 디렉토리 반환
     * 4. getPath()      : getParent() + getName()
     * 5. lastModified() : 최종수정일을 long 타입으로 반환 (타임스탬프 반환)
    ...
    // JAVA_HOME 모든 디렉터리와 파일 File 객체로 가져오기
